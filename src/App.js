...
class App extends Component {
  ...
  componentDidMount() {
    const {time, gameInProgress} = this.state
  ...
  tick = () => {
    ...
    if (time > 0 && gameInProgress) {
      ...
      clearInterval(this.timerId)
      this.setState(prevState => ({gameInProgress: false}))
    }
  }
  ...
  clickImage = imageUrl => {
    const {score, url, gameInProgress} = this.state
    if (imageUrl === url) {
      const totalScore = score + 1
      const newLink = this.imageUrlLink()
      this.setState({score: totalScore, url: newLink})
    } else {
  ...
  clickPlayAgain = () => {
    const {score, gameInProgress, time} = this.state
    clearInterval(this.timerId)
    this.setState({gameInProgress: true, score: 0, time: 60})
  }

  onFilterImages = () => {
    const {activeId} = this.state
    const thumbnailList = imagesList.filter(each => each.category === activeId)
    return thumbnailList
  }

  render() {
    const {score, time, url, activeId, gameInProgress} = this.state
    const filteredImages = this.onFilterImages()
    return (
        ...
        <div className="main-container">
          {gameInProgress ? (
            <div className="game-container">
              <img src={url} alt="match" className="random-image" />
              ...
              <ul className="filteredImages-container">
            ...
            <ScoreCard score={score} clickPlayAgain={this.clickPlayAgain} />
          )}
        </div>
      </div>
    )
